  ...
  try {
    ...
    if (arg === 'help') {
      if (mjml_maj_ver >= 4) {
        ...
        console.log(
          'Run command: NODE_PATH=node_modules node tcpserver.js ' +
...
function handleConnection(conn) {
  ...
  conn.on('data', function (d) {
    ...
    if (data_size === undefined)
      data_size = parseInt(total_data.slice(0, header_size)) + header_size;
      ...
      try {
        ...
        if (typeof result === 'object') {
          if (result.errors.length)
            throw { message: JSON.stringify(result.errors, null, 2) };
          result = result.html;
        }
        conn.write('0');
      } catch (err) {
    ...
    conn.write(
      (
        Array(header_size + 1).join('0') + Buffer.byteLength(result).toString()
      ).slice(-9),
    );
    conn.write(result);
    total_data = '';
