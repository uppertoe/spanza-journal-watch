#!/usr/bin/env python3
"""
ops/aws_setup.py — Provision all AWS resources for a Journal Watch deployment.
...
Run with a named AWS profile (credentials stay in ~/.aws, never in env):
  python ops/aws_setup.py --profile jw-admin --bucket my-jw-bucket --domain yourdomain.com
...
What this script does:
  ...
  - Generates access keys and prints them for .env / /etc/restic/env
...
What requires manual steps:
  ...
  - Creating SES SMTP credentials (for backup email notifications)
"""
...
def print_credentials(keys, bucket, region, config_set_name, webhook_secret=""):
    ...
    if "backup" in keys:
        ...
        print("  RESTIC_PASSWORD is generated by you or by ops/systemd/install.sh, not by AWS.")
